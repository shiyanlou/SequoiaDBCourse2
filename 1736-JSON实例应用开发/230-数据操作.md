...
将代码样例粘贴到 Insert 类 insertOneStr 方法 code 1 的 TODO ~ TODO END 位置区间内。
...
复制以下代码样例，准备向集合内插入1条数据，数据为 BsonObject 对象。用户构造 BsonObject 对象时，通过 put 方法将值写入，key 和 value 也是数据中较为常见的对应方式。 构造 BsonObject 对象，调用集合的 insert 方法，就可以将 BsonObject 内的数据成功写入数据库了。
...
将代码样例粘贴到 Insert 类 insertOne 方法 code 2 的 TODO ~ TODO END 位置区间内。
...
将代码样例粘贴到 Insert 类 insertBatch 方法 code 3 的 TODO ~ TODO END 位置区间内。
...
将代码样例粘贴到 Query 类 queryAll 方法 code 4 的 TODO ~ TODO END 位置区间内。
...
将代码样例粘贴到 Query 类 queryByMatch 方法 code 5 的 TODO ~ TODO END 位置区间内。
...
复制以下代码样例，准备更新指定数据的年龄，将 age = 19 的数据改为 age = 20。这里 matcher 是一个匹配条件对象，modifier 是期望修改后的值。最后调用集合的 update 方法，执行修改操作。
...
将代码样例粘贴到 Update 类 update 方法 code 6 的 TODO ~ TODO END 位置区间内。
...
将代码样例粘贴到 Delete 类 delete 方法 code 7 的 TODO ~ TODO END 位置区间内。
