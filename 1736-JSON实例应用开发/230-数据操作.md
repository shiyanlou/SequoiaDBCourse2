...
复制以下代码样例，准备向集合内插入1条数据,数据为JSON格式，数据中包含的信息为编号、姓名和年龄字段对应的值。
...
通过构造一个 BsonObject 对象进行插入数据，这种插入数据的形式更符合 Java 面向对象的理念，在插入前对数据的修改造作更加方便，数据操作过程中也更容易理解和掌握。
...
除了单条插入数据，巨衫数据库 Java SDK 还支持批量插入数据。具体形式为，在一个容器内存入多个 BsonObject 对象，然后执行插入操作。批量插入的方式使得用户插入数据的效率得到了显著提高。
...
复制以下代码样例，准备通过条件过滤，进行查询，这里以查询年龄等于 19 的数据为例。在拼接查询条件的过程中，可以使用各种匹配符，如这里使用的 $et 代表的”值等于“。想了解更多匹配符，可以前往[巨衫官网](http://doc.sequoiadb.com/cn/sequoiadb-cat_id-1464770442-edition_id-0)。
