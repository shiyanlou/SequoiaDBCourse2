...
复制以下代码样例，准备手动制造异常信息，然后通过 try catch 进行抓取。抓取后在进行错误码比对，符合期望错误的情况下，将异常信息打印出来；不符合的情况下，直接抛出异常信息到更外层。这里制造异常信息的方式为重复调用 createCollectionSpace 创建同名集合空间，由于第一次创建成功，第二次创建时会抛出异常，提示集合空间已存在。
...
将代码样例粘贴到 Sample 类 useSample 方法第 22 行的 TODO ~ TODO END 位置区间内。
