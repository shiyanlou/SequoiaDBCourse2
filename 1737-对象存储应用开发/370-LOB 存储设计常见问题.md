...
1）双击打开 CreateHashPartiton 类，在main函数内找到 TODO 分区方式参数设置 。
...
2）在屏幕下方查看结果输出，可以看到集合分区方式为Hash和Hash分区的分布。
...
1）双击打开 CreateRangePartiton 类，在 main() 函数找到行 TODO code1 主集合参数设置。
...
1 )双击打开 GenerateOid 类，找到行 TODO oid生成测试。
...
如果对于空间浪费较敏感，则选择刚好大于“常用lob大小的三分之一”。例如，常见lob大小为300KB，它的三分之一是100KB，则可以将LobPageSize设置为128K。
